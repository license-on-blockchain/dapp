...
class LOB {
    ...
    watchRootContract(contractAddress) {
        const rootContract = LOB.getRootContract(contractAddress);
    ...
    getCertificateText(licenseContractAddress, callback) {
        const licenseContract = LOB.getLicenseContract(licenseContractAddress);
        const rootContractAddressP = promisify(licenseContract.lobRoot);
        const issuerNameP = promisify(licenseContract.issuerName);
        const safekeepintPeriodP = promisify(licenseContract.safekeepingPeriod);
        const liabilityP = promisify(licenseContract.liability);
        Promise.all([rootContractAddressP, issuerNameP, safekeepintPeriodP, liabilityP])
            .then(([rootContractAddress, issuerName, safekeepingPeriod, liability]) => {
                const rootContract = LOB.getRootContract(rootContractAddress);
                rootContract.buildCertificateText(issuerName, licenseContract.address, safekeepingPeriod, liability, callback);
            })
