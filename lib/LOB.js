...
class LOB {
    ...
    watchLicenseContract(licenseContractAddress) {
        ...
        licenseContract.issuer((error, issuerAddress) => {
            ...
            this._ownedLicenseContracts.updateKey(issuerAddress, (set) => {
                set.add(new LicenseContract(licenseContractAddress));
                return set;
            })
        });
     ...
     * @param {function(*, *)} callback
     */
    createLicenseContract(rootContractAddress, issuerName, liability, safekeepingPeriod, issuerCertificate, callback) {
        const rootContract = getRootContract(rootContractAddress);
        rootContract.createLicenseContract(issuerName, liability, safekeepingPeriod, issuerCertificate, callback);
    }
}
