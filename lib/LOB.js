...
class EthNotificationCenter {
    ...
    static onNewReclaim(licenseContractAddress, callback) {
        const licenseContract = getLicenseContract(licenseContractAddress);
        logWeb3Call(licenseContractAddress, "Reclaim.watch");
        // noinspection JSUnresolvedFunction
...
class LOB {
    ...
    watchLicenseContract(licenseContractAddress) {
        ...
        getAccounts((error, accounts) => {
            ...
            EthNotificationCenter.onNewIssuing(licenseContractAddress, (issuing) => {
                this._updateAllBalancesForIssuance(IssuanceLocation.fromComponents(licenseContractAddress, issuing.args.issuanceID));
            });
    ...
    _updateReclaimableBalance(issuanceLocation, originalOwner, currentOwner) {
        const licenseContract = getLicenseContract(issuanceLocation.licenseContractAddress);
        logWeb3Call(issuanceLocation.licenseContractAddress, "reclaimableBalanceBy", [issuanceLocation.issuanceID, currentOwner, originalOwner]);
    ...
    estimateGasTransferLicenseAndAllowReclaim(issuanceLocation, from, to, amount, callback) {
        const licenseContract = getLicenseContract(issuanceLocation.licenseContractAddress);
        logWeb3Call(licenseContract.address, "transferAndAllowReclaim.estimateGas", [issuanceLocation.issuanceID, to, amount]);
    ...
    reclaim(issuanceLocation, reclaimer, from, amount, gasPrice, callback) {
        const licenseContract = getLicenseContract(issuanceLocation.licenseContractAddress);
        logWeb3Call(licenseContract.address, "reclaim", [issuanceLocation.issuanceID, reclaimer, from, amount]);
    ...
    estimateGasReclaim(issuanceLocation, reclaimer, from, amount, callback) {
        const licenseContract = getLicenseContract(issuanceLocation.licenseContractAddress);
        logWeb3Call(licenseContract.address, "reclaim.estimateGas", [issuanceLocation.issuanceID, reclaimer, from, amount]);
    ...
    getIssuerCertificate(licenseContractAddress, callback) {
        const licenseContract = getLicenseContract(licenseContractAddress);
        logWeb3Call(licenseContractAddress, "issuerCertificate", []);
