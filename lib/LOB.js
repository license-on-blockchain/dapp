...
class LOB {
    ...
    watchLicenseContract(contractAddress) {
        ...
        const updatedIssuances = new Set();

        for (const address of web3.eth.accounts) {
            licenseContract.relevantIssuancesCount(address, (error, relevantIssuancesCount) => {
                if (error) { handleUnknownEthereumError(error); return; }
                for (let i = 0; i < relevantIssuancesCount; i++) {
                    licenseContract.relevantIssuances(address, i, (error, relevantIssuanceID) => {
                        if (updatedIssuances.has(relevantIssuanceID.toNumber())) {
                            return; // Don't fetch the balance twice
                        }
                        updatedIssuances.add(relevantIssuanceID.toNumber());
                        this.updateBalancesForIssuance(licenseContract, relevantIssuanceID);
                    });
                }
            })
        }
        ...
        licenseContract.Issuing({}, {fromBlock: 'latest'}).watch((error, issuing) => {
            ...
            this.updateBalancesForIssuance(licenseContract, issuing.args.issuanceID);
        });
    ...
    updateBalancesForIssuance(licenseContract, issuanceID) {
        getAccounts((error, accounts) => {
            if (error) { handleUnknownEthereumError(error); return; }
