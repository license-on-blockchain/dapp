...
class LOB {
    ...
    _getLicenseContractAddressesForRootContracts(rootContractAddresses) {
    ...
    watchAccountBalance(accounts) {
        ...
        if (accounts.length > 0) {
            return this._watchRootContractForBalances(RootContracts.getAddresses(), accounts);
        } else {
            return Promise.all([]);
        }
    }
    ...
    _watchRootContractForBalances(rootContractAddresses, accounts) {
        const promise = this._getLicenseContractAddressesForRootContracts(rootContractAddresses).then((addresses) => {
