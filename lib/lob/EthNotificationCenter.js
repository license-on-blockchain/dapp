...
export class EthNotificationCenter {
     ...
     * @returns {{stopWatching: function(callback)}} A handle to stop watching if the event is of no more interest
     */
    ...
    static onNewTransfer(licenseContractAddress, accounts, callback, fromBlock = 'latest') {
        ...
        eventHandle.watch((error, event) => {
            ...
            if (accounts.indexOf(event.args.from) === -1 && accounts.indexOf(event.args.to) === -1) {
                return;
            }
            callback(event);
        });
