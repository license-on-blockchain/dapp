...
export class EthNotificationCenter {
    ...
    static onNewIssuing(licenseContractAddress, callback) {
        ...
        licenseContract.Issuing({}, {}).watch((error, issuing) => {
    ...
    static onNewTransfer(licenseContractAddress, callback, fromBlock = 'latest') {
        const licenseContract = getLicenseContract(licenseContractAddress);
        // noinspection JSUnresolvedFunction
        licenseContract.Transfer({}, {fromBlock}).watch((error, transfer) => {
            if (error) { handleUnknownEthereumError(error); return; }
            callback(transfer);
        });
    }
    ...
    static onNewReclaim(licenseContractAddress, callback, fromBlock = 'latest') {
        const licenseContract = getLicenseContract(licenseContractAddress);
        // noinspection JSUnresolvedFunction
        licenseContract.Reclaim({}, {fromBlock}).watch((error, transfer) => {
            if (error) { handleUnknownEthereumError(error); return; }
            callback(transfer);
        });
    }
    ...
    static onRevoke(issuanceLocation, callback) {
        ...
        // noinspection JSUnresolvedFunction
        licenseContract.Revoke({issuanceID: issuanceLocation.issuanceID}, {}).watch((error, revoke) => {
            if (error) { handleUnknownEthereumError(error); return; }
    ...
    static onLicenseContractCreation(rootContractAddress, callback, fromBlock = 'latest') {
        ...
        // noinspection JSUnresolvedFunction
        rootContract.LicenseContractCreation({}, {fromBlock}).watch((error, creation) => {
            if (error) { handleUnknownEthereumError(error); return; }
            callback(creation);
        });
    }
