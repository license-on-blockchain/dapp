...
class LicenseIssuingGasEstimator {
     ...
     * @param {function(*, number)} callback A gas estimate of 0 means that the transaction will most likely fail
     */
    issueLicense(licenseContractAddress, description, code, amount, initialOwnerAddress, initialOwnerName, auditRemark, auditTime, from, issuanceFee, callback) {
        const licenseContract = getLicenseContract(licenseContractAddress);
     ...
     * @param {function(*, number)} callback A gas estimate of 0 means that the transaction will most likely fail
     */
    revokeIssuance(licenseContractAddress, issuanceNumber, from, callback) {
        const licenseContract = getLicenseContract(licenseContractAddress);
        licenseContract.revoke.estimateGas(issuanceNumber, {from, gas: defaultGasEstimate}, (error, estimate) => {
            if (!error && estimate === defaultGasEstimate) {
                estimate = 0;
            }
            callback(error, estimate);
...
export class LicenseIssuing {
     ...
     * @param {function(*, *)} callback
     */
    issueLicense(licenseContractAddress, description, code, amount, initialOwnerAddress, initialOwnerName, auditRemark, auditTime, from, issuanceFee, gasPrice, callback) {
        const licenseContract = getLicenseContract(licenseContractAddress);
        licenseContract.issueLicense(description, code, initialOwnerAddress, amount, auditRemark, auditTime, {from, gasPrice, value: issuanceFee}, (error, transactionHash) => {
            if (!error) {
                this._transactions.addPendingLicenseIssuing(licenseContractAddress, description, code, amount, initialOwnerAddress, initialOwnerName, auditRemark, auditTime, from, transactionHash);
            }
            callback(error, transactionHash);
        });
    }
     ...
     * @param {function(*, *)} callback
     */
    revokeIssuance(licenseContractAddress, issuanceNumber, from, gasPrice, callback) {
        const licenseContract = getLicenseContract(licenseContractAddress);
        licenseContract.revoke(issuanceNumber, {from, gasPrice}, (error, transactionHash) => {
            if (!error) {
