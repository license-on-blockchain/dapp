...
export class LicenseIssuing {
    ...
    issueLicense(licenseContractAddress, description, code, amount, initialOwnerAddress, initialOwnerName, auditRemark, auditTime, from, fee, gasPrice, callback) {
        const licenseContract = getLicenseContract(licenseContractAddress);
        licenseContract.issueLicense(description, code, initialOwnerName, amount, auditRemark, auditTime, initialOwnerAddress, {from, gasPrice, value: fee}, (error, transactionHash) => {
            if (!error) {
    ...
    revokeIssuance(licenseContractAddress, issuanceID, from, gasPrice, callback) {
        const licenseContract = getLicenseContract(licenseContractAddress);
        licenseContract.revoke(issuanceID, {from, gasPrice}, (error, transactionHash) => {
            if (!error) {
                this._transactions.addPendingIssuanceRevoke(licenseContractAddress, issuanceID, from, transactionHash);
            }
            callback(error, transactionHash);
        });
    }
