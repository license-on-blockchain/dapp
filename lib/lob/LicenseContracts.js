...
export class LicenseContracts {
    constructor() {
        this._licenseContracts = new Mongo.Collection('licenseContracts', {connection: null});

        if (Meteor.isClient) {
            new PersistentMinimongo2(this._licenseContracts, 'lob_wallet');
        }
     ...
     * @private
     */
    _getField(licenseContractAddress, fieldName) {
    ...
    getRootContract(licenseContractAddress) {
        return this._getField(licenseContractAddress, 'lobRoot');
    }
    ...
    getIssuerAddress(licenseContractAddress) {
        return this._getField(licenseContractAddress, 'issuer');
    }
    ...
    getSignature(licenseContractAddress) {
        const signature = this._getField(licenseContractAddress, 'signature');
        if (signature !== null && signature !== '0x') {
    ...
    setInternalName(licenseContractAddress, internalName) {
        ...
        this._licenseContracts.upsert({address: licenseContractAddress}, {$set: {
    ...
    registerLicenseContract(licenseContractAddress) {
        if (!this._licenseContracts.findOne({address: licenseContractAddress})) {
            this._licenseContracts.insert({address: licenseContractAddress});
        }
