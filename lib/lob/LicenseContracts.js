...
export class LicenseContracts {
        ...
        this._licenseContracts.find({
            ...
            added(licenseContract) {
                const signingEventHandle = EthNotificationCenter.onLicenseContractSigning(licenseContract.address, (event) => {
                    ...
                    signingEventHandle.stopWatching((error) => {
                ...
                const disablingEventHandle = EthNotificationCenter.onLicenseContractDisabling(licenseContract.address, (event) => {
                    ...
                    disablingEventHandle.stopWatching((error) => {
                        if (error) { handleUnknownEthereumError(error); }
                    });
                });
            }
     ...
     * @param {*} notLoadedValue The value to return if the field has not been loaded yet
     * @return {*}
     * @private
     */
    _getField(licenseContractAddress, fieldName, transform = (x) => x, notLoadedValue = null) {
        if (!web3.utils.isAddress(licenseContractAddress)) {
            debugger;
        ...
        if (cachedValue) {
            ...
            licenseContract.methods[fieldName]().call().then((value) => {
    ...
    /**
     * @param {string} licenseContractAddress
     * @return {string|null|undefined} The signature of the given license contract as a binary string or `null` if the
     *                                 license contract has not been signed or `undefined` if the signature is not
     *                                 loaded yet
     */
    ...
    isSSLCertificateValid(licenseContractAddress) {
        ...
        let signDate = this.getSignDate(licenseContractAddress);
        if (!certificateChain.verifyCertificateChain(signDate)) {
    ...
    getSignatureValidationError(licenseContractAddress) {
        try {
            const signatureValid = this.isSignatureValid(licenseContractAddress);
    ...
    getIssuanceFee(licenseContractAddress) {
        if (this._issuanceFees[licenseContractAddress] === undefined) {
            ...
            const licenseContract = getLicenseContract(licenseContractAddress);
            licenseContract.methods.issuanceFee().call().then((value) => {
                // TODO: Watch for changes of the issuance fee
                this._issuanceFees[licenseContractAddress].set(value);
            }).catch(handleUnknownEthereumError);
        }
        return this._issuanceFees[licenseContractAddress].get() || "0";
    }
    ...
    getManagerAddress(licenseContractAddress) {
        if (this._managerAddresses[licenseContractAddress] === undefined) {
            this._managerAddresses[licenseContractAddress] = new ReactiveVar(null);
            const licenseContract = getLicenseContract(licenseContractAddress);
            licenseContract.methods.managerAddress().call().then((value) => {
                // TODO: Watch for changes of the manager address
                if (value === "0x0000000000000000000000000000000000000000") {
                    value = null;
                }
                this._managerAddresses[licenseContractAddress].set(value);
            }).catch(handleUnknownEthereumError);
        }
        return this._managerAddresses[licenseContractAddress].get();
    }
    ...
    getLicenseContractOfRootContract(rootContractAddress, rootContractIndex) {
        ...
        if (cachedAddress) {
            ...
            return new Promise((resolve, reject) => {
                ...
                rootContract.methods.licenseContracts(rootContractIndex, (error, licenseContractAddress) => {
                    if (error) {
                        reject(error);
                    } else {
