...
export class Balances {
    ...
    /**
     * Update the balance of the given address for the given issuance.
     *
     * @param {IssuanceID} issuanceID The issuance for which the balance should be updated
     * @param {string[]|string} accounts The address for which the balance should be updated
     * @return {Promise} A promise without a return value that is fulfilled when the balance has been updated
     */
    updateBalanceForIssuance(issuanceID, accounts) {
        if (!Array.isArray(accounts)) {
            accounts = [accounts];
        }
        ...
        for (const account of accounts) {
            ...
            const updateReclaimableBalancePromise = this._getReclaimOrigins(issuanceID, account).then((reclaimOrigins) => {
                const promises = [];
                for (const reclaimOrigin of reclaimOrigins) {
                    const promise = this._updateTemporaryBalance(issuanceID, account, reclaimOrigin);
                    promises.push(promise);
                }
                return Promise.all(promises);
            });
    ...
    updateAllRelevantBalancesForLicenseContract(licenseContractAddress, accounts) {
        const promises = [];
        for (const address of accounts) {
            const promise = this._getRelevantIssuanceIDs(licenseContractAddress, address).then((relevantIssuanceNumbers) => {
                const promises = [];
                for (const relevantIssuanceNumber of relevantIssuanceNumbers) {
                    const issuanceID = IssuanceID.fromComponents(licenseContractAddress, relevantIssuanceNumber);
                    const promise = this.updateBalanceForIssuance(issuanceID, address);
                    promises.push(promise);
                }
                return Promise.all(promises);
            });
     ...
     * Listen for changing balances on the given license contract.
     *
     * @param {IssuanceID} issuanceID The issuance to watch
     * @param {string[]} accounts The accounts to update the balances for
     */
    watchIssuanceForNewBalance(issuanceID, accounts) {
        // TODO: Filter for the correct issuanceNumber based on events in EthNotificationCenter
        EthNotificationCenter.onNewTransfer(issuanceID.licenseContractAddress, accounts, (transfer) => {
            const issuanceNumber = transfer.args.issuanceNumber.toNumber();
            if (issuanceNumber !== issuanceID.issuanceNumber) {
                return;
            }
            const from = transfer.args.from;
            const to = transfer.args.to;
        ...
        // TODO: Filter for the correct issuanceNumber based on events in EthNotificationCenter
        EthNotificationCenter.onNewReclaim(issuanceID.licenseContractAddress, accounts, (reclaim) => {
            const issuanceNumber = reclaim.args.issuanceNumber.toNumber();
            if (issuanceNumber !== issuanceID.issuanceNumber) {
                return;
            }
