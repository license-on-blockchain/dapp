...
export class Balances {
    ...
    getBalance(issuanceLocation, accounts) {
        ...
        return this._balancesCollection.find(query)
            .map((doc) => doc.balance)
            .reduce((x, y) => x + y, 0);
    }
    ...
    getOwnedBalance(issuanceLocation, accounts) {
        ...
        return this._balancesCollection.find(query)
            .map((doc) => ((doc.balance || 0) - (doc.borrowedBalance || 0)))
            .reduce((x, y) => x + y, 0);
    }
    ...
    _setBalance(issuanceLocation, account, newBalance) {
        ...
        if (!this._balancesCollection.findOne(query)) {
            this._balancesCollection.insert(query);
        }
        this._balancesCollection.update(query, {$set: {balance: newBalance}});
    }
    ...
    _setBorrowedBalance(issuanceLocation, account, newBalance) {
        ...
        this._balancesCollection.update(query, {$set: {borrowedBalance: newBalance}});
    }
    ...
    getReclaimableIssuanceLocations(accounts) {
        ...
        return this._openReclaimsCollection.find({reclaimer: {$in: accounts}, balance: {$gt: 0}})
            .map((doc) => {
                return IssuanceLocation.fromComponents(doc.licenseContract, doc.issuanceID);
            });
    }
    ...
    getReclaimOrigins(issuanceLocation, accounts) {
        ...
        return this._openReclaimsCollection.find(query)
    ...
    getReclaimableBalanceFrom(issuanceLocation, accounts, currentOwner) {
        ...
        const doc = this._openReclaimsCollection.findOne(query);
        if (doc) {
            return doc.balance;
        } else {
            return 0;
    ...
    _setReclaimableBalanceFrom(issuanceLocation, reclaimer, currentOwner, newReclaimableBalance) {
        ...
        this._openReclaimsCollection.upsert(query, data);
    }
