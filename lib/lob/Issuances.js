...
export class Issuances {
     ...
     * @returns {{licenseContract: string, issuanceNumber: number, description: string, code: string, initialOwnerName: string, originalSupply: number, auditTime: number, auditRemark: string, revoked: boolean}}
     */
    getIssuance(issuanceID) {
        const query = {licenseContract: issuanceID.licenseContractAddress, issuanceNumber: issuanceID.issuanceNumber};
    ...
    _fetchIssuanceMetadata(issuanceID, callback) {
        ...
        licenseContract.issuances(issuanceNumber, (error, rawIssuanceData) => {
            ...
            callback({
                ...
                initialOwnerName: "TODO: No longer exists",
