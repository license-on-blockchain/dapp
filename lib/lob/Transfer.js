...
export class Transfer {
    ...
    transferLicense(issuanceLocation, from, to, amount, gasPrice, callback) {
        ...
        return licenseContract.transfer(issuanceLocation.issuanceID, to, amount, { from, gasPrice }, (error, value) => {
            // Internally temporarily update the balance for this account for faster perceived feedback
            if (!error) {
                const oldBalance = this.balances.getBalanceForIssuanceLocation(issuanceLocation).getBalance(from);
                this.balances.getBalanceForIssuanceLocation(issuanceLocation).setBalance(from, oldBalance.minus(amount));
            }
