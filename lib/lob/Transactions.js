...
export class Transactions {
    constructor() {
        ...
        this._transactions.find({blockNumber: null}).observe({
            added(transaction) {
                ...
                web3.eth.getTransaction(transaction.transactionHash).then((web3Transaction) => {
                ...
                switch (transaction.transactionType) {
                    case TransactionType.Transfer:
                        eventHandle = EthNotificationCenter.onNewTransfer(transaction.licenseContract, [transaction.from], (event) => {
                    ...
                    case TransactionType.Reclaim:
                        eventHandle = EthNotificationCenter.onNewReclaim(transaction.licenseContract, [transaction.from], (event) => {
                            if (event.transactionHash === transaction.transactionHash) {
                                eventHandle.stopWatching((error) => {
                                    if (error) { handleUnknownEthereumError(error); }
                                });
                    ...
                    case TransactionType.LicenseContractCreation:
                        eventHandle = EthNotificationCenter.onLicenseContractCreation(transaction.rootContract, (event) => {
                            if (event.transactionHash === transaction.transactionHash) {
                                ...
                                transactions.update({transactionHash: event.transactionHash}, {$set: {
                                    blockNumber: event.blockNumber,
                                    licenseContract: event.returnValues.licenseContractAddress
                                }});
                            }
                        }, transaction.watchForMiningCheckpoint);
                        break;
    ...
    getPendingIssuances(licenseContractAddress) {
        return this._transactions.find({
            transactionType: TransactionType.LicenseIssuing,
            licenseContract: licenseContractAddress,
            blockNumber: null,
