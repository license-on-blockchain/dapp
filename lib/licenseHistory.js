...
function computeRelevantAddresses(snapshots, transfers, initiallyRelevantAccounts) {
    ...
    for (let t = snapshots.length - 1; t >= 0; t--) {
        ...
        if (relevantAddresses[t+1].has(transfer.returnValues.to)) {
            relevantAddresses[t].add(transfer.returnValues.from);
        }
    }
...
export function drawLicenseHistory(canvas, transfers, issuanceID, issuerName) {
    addressRows = {
    ...
    const initialOwnerAddress = transfers[0].returnValues.to;
    ...
    for (let t = 0; t < snapshots.length; t++) {
        ...
        for (const [address, balance] of Object.entries(snapshot)) {
            const {from, to, amount} = transfers[t].returnValues;
            const reclaim = transfers[t].event === "Reclaim";
            const temporary = transfers[t].event === "Transfer" && transfers[t].returnValues.temporary === true;
