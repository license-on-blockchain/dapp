...
export function drawLicenseHistory(canvas, transfers, issuerName, initialOwnerName) {
    const snapshots = lob.computeBalanceSnapshots(transfers);
    const initialOwnerAddress = transfers[0].args.to;

    const relevantAddresses = computeRelevantAddresses(snapshots, transfers, lob.accounts.get());
    ...
    drawText(context, snapshots.length + 1, 0.2, "(" + TAPi18n.__("license_history.license_contract") + ": " + licenseContractAddress + ")", 'right');
    ...
    for (const [address, balance] of Object.entries(snapshots[snapshots.length - 1])) {
        ...
        if (lob.accounts.get().indexOf(address) !== -1) {
            addressDescriptions.push(TAPi18n.__("license_history.your_address"));
        }
