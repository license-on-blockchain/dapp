...
export class CertificateChain {
     ...
     * @param {Date} [verifyExpirationDate] Check whether the current date is within the leaf certificate's validity
     *                                      date range (not before to not after). By default the current date is used.
     ...
     */
    verifyCertificateChain(verifyExpirationDate) {
        if (!verifyExpirationDate) {
            verifyExpirationDate = new Date();
        }
        try {
            return forge.pki.verifyCertificateChain(caStore, this.certificates, {validityCheckDate: verifyExpirationDate});
        } catch (error) {
            throw error.message;
        }
    }
    ...
    getLeafCertificate() {
        return this.getCertificateAtIndex(this.getChainLength() - 1);
    }
