...
export const Marketplace = {
    ...
    getOffer(issuanceID, seller) {
        seller = seller.toLowerCase();
        const filtered = this._offers.offers.get().filter((offer) => {
            return offer.issuanceID === issuanceID && offer.seller === seller;
        });
        let offer = (filtered.length > 0) ? filtered[0] : null;

        if (!offer && !this._offers.upToDate && !this._offers.fetchInProgress) {
            this._refreshOffers();
        }
